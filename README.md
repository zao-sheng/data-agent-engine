...
| 本体存储 | 读取层抽象（YAML 默认 / SQLite 编译产物 / Supabase 真源已实现）：`ontology_store` 接口，换存储只动读取层；`ontology_compile`（YAML→SQLite）、`ontology_import`（YAML→Supabase 批量导入，键对齐+类型安全）、`ontology_export`（Supabase→YAML 评审副本）、`SupabaseOntologyWriter`（多人编辑+乐观锁）；`ontology_register` 工具（建模流程确认后写 YAML/Supabase 双通道）；建表脚本 `backend/supabase/schema.sql`、RLS 脚本 `backend/supabase/rls.sql` |
